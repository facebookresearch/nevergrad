...
(echo '\begin{itemize}' ; ./scripts/compare.sh NGOpt NgIohLM | sed 's/rnk__//g' | sed 's/^/\\item /g' | sed 's/(x)//g' |sed 's/_/-/g' ; echo '\item []' ; echo '\end{itemize}' ) > compa.tex
(echo '\begin{itemize}' ; ./scripts/compare.sh NGOpt NgIohLM | sed 's/rnk__//g' | sed 's/^/\\item /g' | sed 's/(x)//g' |sed 's/_/-/g' ; echo '\item []' ; echo '\end{itemize}' ) > compa2.tex
