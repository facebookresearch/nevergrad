...
def draw(data, filename, context):
    for n in [3, 7, 15]:
        ...
        for i, a in enumerate(sortedalgorithms):
            if (
                i < len(algorithms) - n and algorithms[i] != "Zero" and algorithms[i] != "StupidRandom"
            ):  # and i < len(algorithms) - n and (i % num != 0):
...
def go(d, n, budget, max_algos=max_algos, stop_on_error=False):
    ...
    optims = [
        x
        for x in list(ng.optimizers.registry.keys())
        if len(x.replace("OnePlusOne", "1+1")) < 10 and x not in ["NgIoh" + str(i) for i in range(50)]
    ]
    ...
    for realbudget in [budget // 4, budget // 2, budget, 2 * budget, 4 * budget]:
        res = Parallel(n_jobs=nj)(
            delayed(subgo)(a, d, n, realbudget, stop_on_error=stop_on_error) for a in optims
        )
    ...
    filename = (
        f"perf{label_type}x_d{d}_n{n}_budget{budget}______rs{resampling}_{np.random.randint(200)}_{nj}.txt"
    )
    ...
    filename = (
        f"overfit{label_type}x_d{d}_n{n}_budget{budget}______rs{resampling}_{np.random.randint(200)}_{nj}.txt"
    )
    with open(filename, "w") as f:
...
# n += [n_]
# budget += [b_]
d_ = np.random.choice([3, 10, 100, 1000, 10000, 100000])  # , 1000000])
