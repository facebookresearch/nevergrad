...
grep 'Algo.*seed.*__result' ngrob*.out | grep "Game${pb}_" | grep -v 'fp__' |  sed 's/.*://g' | sed 's/fpll_/fplL/g' | sed 's/fipl_/fpl/g' | sed 's/fip__/fp/g' | sed 's/_/ /g' | awk '{ print "data[\"", $2, "\"][", $3, "] += [", $4, "]" }' | sed 's/budget//g' | sed 's/ loss//g' >> plotter.py
...
echo 'plt.savefig("ngrobust' $pb '.png".replace(" ",""))' >> plotter.py
