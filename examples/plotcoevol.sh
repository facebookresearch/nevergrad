...
grep 'Algo.*seed.*__result' coevol_*.out | grep `echo seed${s}_ | sed 's/seed_/./g' ` | sed 's/.*://g' | sed 's/fp___/fp/g' | sed 's/_/ /g' | awk '{ print "data[\"", $1, "\"][", $2, "] += [", $3, "]" }' | sed 's/budget//g' | sed 's/ loss//g' >> plotter.py
