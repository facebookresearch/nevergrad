...
class VectorLinkedList:
    ...
    def extend(self, nodes: List[VectorNode], index: int) -> None:
        """ Extends the Multilist with a list of nodes at
        `index` position"""
        for node in nodes:
...
class HypervolumeIndicator:
    ...
    def compute(self, points: List[List[float]]) -> float:
    ...
    def sort_by_index(self, node_list: List[VectorNode], dimension_index: int) -> List[VectorNode]:
