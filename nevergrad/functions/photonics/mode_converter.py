...
def mode_converter(X, ev_out=-3.5**2/4, ev_in=-3.5**2, plot=False, show=False):
    """
    ...
    MODE = 'E' # 'E' or 'H'
    # TODO: change mode choice
    # MODE_IN = 1 # MODE NUMBER INPUT PORT 
    # MODE_OUT = 1 # MODE NUMBER OUTPUT PORT
    ...
    assert (len(X) == nb_struct_x and len(X[0]) == nb_struct_y),   f"nb_struct_x={nb_struct_x}, nb_struct_y={nb_struct_y}, np.shape(X)={np.shape(X)}"

    #  CENTER WAVEGUIDE
    X = 1 + X * (centerWG_n**2-1) # shift value range from 0-1 to 1-n
    ...
    if plot:
        ...
        plt.pcolor(xa2, ya2, np.sqrt(ER2.T))
        plt.colorbar()
    ...
    inv_uryy = sp.diags_array(1/UR2[nx, 0:Ny2:2], format="csc")
    ...
    if plot:
        plt.subplot(gs[:2, :2])
        plt.plot(np.real(input_mode), ya)
        plt.xlabel('Real(U)'),
        plt.ylabel('y ($\mu$m)')
        plt.title(f"Input Mode, neff = {np.round(neff_in,3)}")
    ...
    if plot:
        plt.subplot(gs[:2, 2:])
        plt.plot(np.real(output_mode), ya)
        plt.xlabel('Real(U)')
        plt.ylabel("y ($\mu$m)")
        plt.title(f"Output Mode, neff = {np.round(neff_out,3)}")
        plt.tight_layout()
        plt.savefig("Testing_shapes.png")
    ...
    #aref = input_mode\fref[:]
    #atrn = output_mode\ftrn[:]
    # CALCULATE REFLECTANCE AND TRANSMITTANCE
    #R = abs(aref[0])^2
    #T = abs(atrn[0])^2

    # CALCULATE S PARAMETERS
    S11 = fref @ np.conj(input_mode) / (np.conj(input_mode).T @ input_mode)
    S21 = ftrn @ np.conj(output_mode) / (np.conj(output_mode).T @ output_mode)
    SR = abs(S11)**2  # REFLECTION
    ...
    if plot:
        plt.figure(1)
        plt.subplot(2, 1, 2)
        plt.pcolormesh(xa, ya, np.real(U).T / np.max(np.abs(U)))
        plt.contour(xa2, ya2, np.sqrt(ER2.T))
        plt.xlabel('x ($\\mu$m)')
        plt.ylabel('y ($\\mu$m)')
        plt.colorbar()

        plt.subplot(2,1,1)
        plt.pie([SR, ST, 1-SR-ST])
        plt.legend(['S_R','S_T','Loss'])
        plt.tight_layout()
        plt.savefig("Testing_Ceviche_modeconverter.png")
        if show:
            plt.show()
    return 1-ST, U
...
if __name__ == "__main__":
    ...
    nb_struct_x = int(centerWG_L / dx2)
    nb_struct_y = int(centerWG_w / dy2)
    X = X = np.ones((nb_struct_x, nb_struct_y))#(np.random.random((nb_struct_x, nb_struct_y))+1)**2
    print(nb_struct_x, nb_struct_y)
    ST, field = mode_converter(X, ev_out=-inputWG_n**2/4, ev_in=-inputWG_n**2, plot=True, show=False)
    print(ST)
