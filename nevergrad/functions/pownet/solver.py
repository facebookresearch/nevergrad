...
def download_dataset(dataset_name="cambodian", force_update=False):
    root_dataset_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'pypownet', 'pypownetr', 'datasets')
    dataset_paths = {
        "cambodian": {
    ...
    if not os.path.exists(dataset_paths[dataset_name]['path']) or force_update:
        ...
        urllib.request.urlretrieve(
            f"https://github.com/pacowong/pypownet/raw/main/pypownetr/datasets/{dataset_paths[dataset_name]['file']}", datasetZipPath)
...
def pownet_solver(location, year=2016, start_day=1, end_day=2):
    ...
    answers = solve_pownet_subproblems(pyomo_model, model_data_path, solver_func=solver_func, start_day=start_day, last_day=end_day)
    return answers
