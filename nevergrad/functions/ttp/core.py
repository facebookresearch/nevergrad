...
class TTPInstance(base.ExperimentFunction):
    ...
    def __init__(self, file: str = "./instances/a20_n95_01.ttp") -> None:
        ...
        instru = ng.p.Instrumentation( tspTour = p1, 
                                        packingPlan = p2
                                        ).set_name("")
    ...
    def evaluate(self, tspTour: np.ndarray, 
                    packingPlan: np.ndarray) -> float:
        ...
        itemsPerCity = int( len(solution.packingPlan) 
                            / ( len(solution.tspTour) - 2))
        ...
        for i in range(0, len(tour) - 1):
            ...
            if i > 0:
                for itemNumber in range (0, itemsPerCity):
                    ...
                    itemIndex = (currentCity + 
                                itemNumber * (self.numberOfNodes-1)
                                )
            ...
            distance = int(math.ceil(self.distances(tour[i], 
                                                    tour[h] 
                                                    )))
        ...
        if wc > self.capacityOfKnapsack:
            solution.ob = (solution.ob 
                            + 1000 
                            * (wc-self.capacityOfKnapsack)
                            )
