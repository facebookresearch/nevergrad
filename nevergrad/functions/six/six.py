...
class Six:
    ...
    def consistency(self, verbose:bool = False) -> None:
        ...
        assert sum(len(p) for p in self.players) + len(self.cards) + sum(len(c) for c in self.current) + len(self.past) == 104
    ...
    def cost(self, i: int) -> int:
        return 7 if (i % 11 == 0) else 5 if (i % 10 == 0) else 3 if (i % 5 == 0) else 1
    ...
    def play_moves(self, moves: list, preserve: bool = False, valued_function: int = -1) -> list:
        ...
        costs = np.array([0. for _ in range(self.num_players)])
    ...
    def naive_play(self, player: int, use_ai: bool = False) -> int:
        ...
        for c in my_cards:
            ...
            moves = [c if i == player else played_by_others[(i-1) if i > player else i] for i in range(self.num_players)]
