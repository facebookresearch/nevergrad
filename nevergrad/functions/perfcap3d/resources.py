...
def download_resource(name : str) -> Path:
    ...
    if name not in _NAMED_URLS:
        raise ValueError(f'Resource "{name}" is not available. Please choose among:\n{list(_NAMED_URLS.keys())}')
    ...
    if resource_size != current_file_size:
        ...
        try:
            ...
            prog_bar = tqdm(total = resource_size,unit='B',unit_scale = True, unit_divisor=1024, initial=current_file_size)
...
def decompress_resource(resource_file : Path):
    ...
    try:
        if is_multipart:
            logger.info("This is a multi-part file. Decompression may take a few minutes. Please be patient ...")
...
def prepare_resources():
    ...
    logger.info('Resources OK. In case you experience an execution error consider clearing the resource folder to start from scratch: %s',
