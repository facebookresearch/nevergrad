...
def test_core_function(func: Callable[[np.ndarray], float], expected: float, data: Optional[List[float]]) -> None:
    if data is None:
        data = [0.662, -0.217, -0.968, 1.867, 0.101, 0.575, 0.199, 1.576, 1.006, 0.182, -0.092, 0.466]  # only for non-regression
    value = func(np.array(data))
    np.testing.assert_almost_equal(value, expected, decimal=5)


@testing.parametrized(
    irregularity=(_core.irregularity, [-10, -2, -1, 0, 0.1, 1, 2, 2.5], [-10.426, -2.021, -1, 0, 0.107, 1, 1.988, 2.635]),
    asymmetry_04=(_core.Asymmetry(0.2), [-10, -2, -1, 0, 0.1, 1, 2, 2.5], [-10, -2, -1, 0, 0.092, 1, 2.366, 3.34]),
    asymmetry_02=(_core.Asymmetry(0.4), [-1, 0, 0.1, 1, 2], [-1, 0, 0.086, 1, 2.96]),
    illconditionning_10=(_core.Illconditionning(10.0), [-2, -1, 0, 0.1, 1, 2], [-2.0, -1.259, 0.0, 0.2, 2.512, 6.325]),
    illconditionning_1=(_core.Illconditionning(1.0), [-2, -1, 0, 0.1, 1, 2], [-2, -1, 0, 0.1, 1, 2]),  # no change expected
    translation=(_core.Translation(np.array([1, 2])), [0, 10], [-1, 8]),
    rotation=(_core.Rotation(np.array([[0, 1], [1, 0]])), [0, 10], [10, 0]),
)
def test_transform(func: Callable[[np.ndarray], np.ndarray], data: List[float], expected: List[float]) -> None:
    value = func(np.array(data))
...
def test_split(
    permutation: Iterable[float], dimensions: List[int], overlap: int, expected: Union[Type[Exception], List[np.ndarray]]
) -> None:
