...
class UnitCommitmentProblem(ExperimentFunction):
    ...
    """
    def __init__(self, problem_name='semi-continuous', num_timepoints: int = 13,
        ...
        param_operational_output = ng.p.Array(shape=(self.num_generators, self.num_timepoints)).set_bounds(0, self.p_max)
        ...
        instru = ng.p.Instrumentation(operational_output=param_operational_output, operational_states=param_operational_states).set_name("")
    ...
    def unit_commitment_obj_with_penalization(self, operational_output, operational_states):
        ...
        lb_penalty = np.sum(np.clip(self.p_min*operational_states - operational_output, 0, a_max=None), axis=None)
        ub_penalty = np.sum(np.clip(operational_output - self.p_max*operational_states, 0, a_max=None), axis=None)
        # Running cost
        running_cost = np.sum(np.sum(operational_output, axis=1) * self.cost_a + np.sum(operational_states, axis=1) * self.cost_b)
