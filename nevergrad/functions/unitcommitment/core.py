...
class UnitCommitmentProblem(ExperimentFunction):
    ...
    def __init__(
        ...
        params = {
            x: y for x, y in locals().items() if x not in ["self", "__class__"]
        }  # for copying
        ...
        # Cost
        self.cost_a = np.clip(
            np.random.randn(self.num_generators) * 0.2 + 0.5, a_min=0, a_max=None
        )
        self.cost_b = np.random.uniform(low=0, high=10, size=(self.num_generators,))

        self.penalty_weight = penalty_weight

        param_operational_output = ng.p.Array(
            shape=(self.num_generators, self.num_timepoints)
        ).set_bounds(0, self.p_max)
        # param_operational_states = ng.p.Array(shape=(self.num_generators, self.num_timepoints)).set_bounds(0, 1).set_integer_casting() #
        param_operational_states = ng.p.Choice(
            [0, 1], repetitions=self.num_timepoints * self.num_generators
        )
        ...
        self._descriptors.update(
            num_timepoints=self.num_timepoints, num_generators=self.num_generators
        )

    def unit_commitment_obj_with_penalization(
        self, operational_output, operational_states
    ):
        operational_states = np.array(operational_states).reshape(
            self.num_generators, self.num_timepoints
        )
        demand_penalty, lb_penalty, ub_penalty = 0, 0, 0
        # From demand constraint
        demand_penalty = np.sum(
            np.abs(np.sum(operational_output, axis=0) - self.demands)
        )
        # From semi_continuous_constraints
        lb_penalty = np.sum(
            np.clip(
                self.p_min * operational_states - operational_output, 0, a_max=None
            ),
            axis=None,
        )
        ub_penalty = np.sum(
            np.clip(
                operational_output - self.p_max * operational_states, 0, a_max=None
            ),
            axis=None,
        )
        # Running cost
        ...
        ret = (
            running_cost
            + (demand_penalty + lb_penalty + ub_penalty) * self.penalty_weight
        )
        return ret
