...
@testing.parametrized(
    fft=(True, False, [3, 3, 5, 5]),
    real=(False, False, [4, 5, 5, 4]),
    align=(False, True, [4, 5, 5, 4]),
)
def test_crossover(fft: bool, align: bool, expected: tp.List[int]) -> None:
    x1 = 4 * np.ones((2, 4))
    x2 = 5 * np.ones((2, 4)) if not fft else np.arange(8).reshape((2, 4))
    co = mutation.Crossover(axis=1, fft=fft, align=align)
...
def test_crossover_axis(axis: tp.Optional[tp.Tuple[int, ...]], max_size: tp.Optional[int]) -> None:
    ...
    assert co.name.startswith("Crossover["), f"Unexpected {co.name}"
...
def test_correlate(axes: tp.Any, expected: tp.Any) -> None:
    # TODO check border effects (none now with copy=True)
    x = [[1, 0, 0, 0],
         [2, 0, 0, 0]]
    y = [[0, 0, 4, 0],
         [0, 0, 1, 0]]
