...
from . import instanciate
from .instanciate import LINETOKEN
...
def test_symlink_folder_tree() -> None:
    ...
    with tempfile.TemporaryDirectory() as folder:
        instanciate.symlink_folder_tree(path, folder)
...
class InstanciationTests(TestCase):
    ...
    def test_uncomment_line(self, line: str, ext: str, expected: str) -> None:
        if isinstance(expected, str):
            output = instanciate.uncomment_line(line, ext)
            np.testing.assert_equal(output, expected)
        else:
            np.testing.assert_raises(expected, instanciate.uncomment_line, line, ext)
    ...
    def test_instrumentized_folder(self, clean_copy: bool) -> None:
        ...
        ifolder = instanciate.InstrumentizedFolder(path, clean_copy=clean_copy)
        np.testing.assert_equal(ifolder.dimension, 4)
        np.testing.assert_equal(len(ifolder.instrumentized_files), 1)
        with ifolder.instanciate([1, 2, 3, 4]) as tmp:
...
def test_instanciate_file() -> None:
    path = Path(__file__).parent / "examples" / "basic" / "script.py"
    instanciable = instanciate.InstrumentizedFile(path)
    np.testing.assert_equal(instanciable.dimension, 4)
    np.testing.assert_array_equal(instanciable.variables, [variables.Gaussian(mean=90., std=20.),
...
def test_instrumentized_function() -> None:
    ifunc = instanciate.InstrumentedFunction(_arg_return, variables.SoftmaxCategorical([1, 12]), "constant",
...
def test_instrumentized_function_kwarg_order() -> None:
    ifunc = instanciate.InstrumentedFunction(_arg_return, kw4=variables.SoftmaxCategorical([1, 0]), kw2="constant",
