...
def test_array_as_ascalar() -> None:
    ...
    #  int
    var = variables.ArrayOld(1).asscalar(int)
    ...
    with pytest.raises(RuntimeError):
        variables.ArrayOld(1).asscalar(int).asscalar(float)
    with pytest.raises(RuntimeError):
        variables.ArrayOld(2).asscalar(int)
    with pytest.raises(ValueError):
        variables.ArrayOld(1).asscalar(np.int64)  # type: ignore


def test_array() -> None:
    var = variables.ArrayOld(2, 2).affined(1000000).bounded(3, 5, transform="arctan")
...
@pytest.mark.parametrize(  # type: ignore
    ...
    [
        ...
        (variables.ScalarOld().affined(10, 100).bounded(-200, 200), [0], 2198.7269),
        ...
        (variables.ScalarOld().affined(10, 100).bounded(-200, 200), [-10], 0),
        (variables.ScalarOld().affined(10, 100).bounded(-200, 200, transform="clipping"), [1], 110),
