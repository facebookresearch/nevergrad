...
class _Chain(base.Optimizer):
    ...
    def __init__(
        ...
        assert all(x in ("third", "half", "dimension", "num_workers", "sqrt") or x > 0 for x in self.budgets), str(
            self.budgets)
...
chainNaiveTBPSACMAPowell = Chaining([NaiveTBPSA, CMA, Powell], ["third", "third"]).set_name("chainNaiveTBPSACMAPowell",
                                                                                            register=True)
...
class robust_slow(ASCMADEQRthird):
    ...
    def __init__(self, parametrization: IntOrParameter, budget: tp.Optional[int] = None, num_workers: int = 1) -> None:
        ...
        assert budget is not None
        self.optims = [
            chainNaiveTBPSACMAPowell(self.parametrization, budget=budget, num_workers=num_workers),
            # unfair for Powell...
...
@registry.register
class NGOptRLlm(base.Optimizer):
    """Nevergrad optimizer by competence map. You might modify this one for designing youe own competence map."""
                ...
                if self.has_noise and self.fully_continuous:
                    ...
                    if self.has_discrete_not_softmax or not self.parametrization.descriptors.metrizable or not self.fully_continuous:
                        ...
                        if num_workers > budget / 5:
                            ...
                            # Possibly a good idea to go memetic for large budget, but something goes wrong for the moment.
                            if num_workers == 1 and budget > 5 * self.dimension and self.dimension > 7:  # Let us go memetic.
                                optimClass = chainCMAPowell  # type: ignore
                            else:
                                if num_workers == 1 and budget < self.dimension * 30:
                                    if self.dimension > budget * 10:  # One plus one so good in large ratio "dimension / budget".
                                        optimClass = OnePlusOne  # type: ignore
                                    elif self.dimension < 5:
                                        optimClass = MetaModel  # type: ignore
                                    else:
...
ProgODOPOInf = ConfSplitOptimizer(num_optims=100000, progressive=True,
                                  multivariate_optimizer=OptimisticDiscreteOnePlusOne).set_name("ProgODOPOInf",
                                                                                                register=True)

ProgONOPOInf = ConfSplitOptimizer(num_optims=100000, progressive=True,
                                  multivariate_optimizer=OptimisticNoisyOnePlusOne).set_name("ProgONOPOInf",
                                                                                             register=True)