...
class BenchmarkTests(TestCase):
    ...
    def test_softmax_discretization(self, arity: int, data: List[float], expected: List[float],
    ...
    def test_thresholding_discretization(self, arity: int, data: List[float], expected: List[float]) -> None:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
