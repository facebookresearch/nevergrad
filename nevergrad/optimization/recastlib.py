...
class ScipyMinimizeBase(recaster.SequentialRecastOptimizer):
    ...
    def _optimization_function(self, objective_function: Callable[[base.ArrayLike], float]) -> base.ArrayLike:
        ...
        while remaining > 0:  # try to restart if budget is not elapsed
            ...
            res = scipyoptimize.minimize(objective_function, best_x if not self.random_restart else np.random.normal(0., 1., self.dimension), method=self.method, options=options, tol=0)
            if res.fun < best_res:
                best_res = res.fun
                best_x = res.x
            remaining = budget - self._num_suggestions
