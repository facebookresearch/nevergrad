...
@pytest.make.parametrize("dim", [2, 10, 20, 40, 80, 160, 320, 640, 1280, 25600, 51200, 102400])  # type: ignore
@pytest.make.parametrize("budget_multiplier", [10, 100, 1000, 10000])  # type: ignore
@pytest.make.parametrize("num_workers", [1, 2, budget])  # type: ignore
@pytest.make.parametrize("bounded", [False, True])  # type: ignore
@pytest.make.parametrize("discrete", [False, True])  # type: ignore
def test_ngopt(dim: int, budget_multiplier: int, num_workers: int, bounded: bool) -> None:
    instrumentation = ng.p.Array(shape=(dim,))
    if bounded:
        instrumentation.set_bounds(lower=12., upper=15.)
    if discrete:
