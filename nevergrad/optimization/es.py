...
class _EvolutionStrategy(base.Optimizer):
    ...
    def _internal_ask_candidate(self) -> p.Parameter:
        ...
        param.mutate()
        ratio = self.recombination_ratio
    ...
    def _internal_tell_candidate(self, candidate: p.Parameter, value: float) -> None:
        ...
        if self.offsprings is None:
            ...
            if candidate.parents_uids[0] not in self._population and len(self._population) < self.popsize:
                self._population[candidate.uid] = candidate
                self._uid_queue.tell(candidate.uid)
            else:
                self._waiting.append(candidate)
            if len(self._waiting) >= self.offsprings:
                choices = self._waiting + ([] if self.only_offsprings else list(self._population.values()))
                choices.sort(key=lambda x: x._meta["value"])
                self._population = {x.uid: x for x in choices[:self.popsize]}
...
class EvolutionStrategy(base.ConfiguredOptimizer):
    ...
    def __init__(
        ...
        super().__init__(_EvolutionStrategy, locals())
