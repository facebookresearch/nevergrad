...
#     funcs += [_mlda.Landscape(transform) for transform in [None, "square", "gaussian"]]
#     seedg = create_seed_generator(seed)
#     optims = ["NaiveTBPSA", "ScrHammersleySearch", "PSO", "OnePlusOne", "NGO", "Shiwa", "DiagonalCMA", "CMA", "OnePointDE", "TwoPointsDE", "QrDE", "LhsDE",
...
def bragg_structure(seed: tp.Optional[int] = None) -> tp.Iterator[Experiment]:
    ...
    optims: tp.List[tp.Union[str, ConfiguredOptimizer]] = ["TwoPointsDE", "DE", "CMA", "NaiveTBPSA", "DiagonalCMA", "Shiwa"]
    ...
    for budget in [1e3, 1e4, 1e5, 1e6]:
        xpseed = next(seedg)
        for algo in default_optims if default_optims else optims:
